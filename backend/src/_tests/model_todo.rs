use crate::{
    model::{
        self,
        db::init_db,
        todo::{Todo, TodoPatch, TodoStatus},
    },
...
async fn model_todo_update_ok() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let updated_data_fx = TodoPatch {
        title: Some("test - model_todo_update_ok 2".to_string()),
        ..Default::default()
    };

    // -- ACTION
    let todo_updated = TodoMac::update(&db, &utx, todo_fx.id, updated_data_fx.clone()).await?;

    // -- CHECK
    let todos = TodoMac::list(&db, &utx).await?;
    assert_eq!(3, todos.len());
    assert_eq!(todo_fx.id, todo_updated.id);
    assert_eq!(updated_data_fx.title.unwrap(), todo_updated.title);
